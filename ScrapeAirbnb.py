...
def singlestar(index, soup):
    ...
    full_star = stars.find_all("i", {"class" : "icon icon-beach icon-star"})
    half_star=  stars.find_all("i", {"class" : "icon icon-beach icon-star-half"})
...
def getAmenitiesList(tree, ListingID):
    ...
    try:
        ...
        if len(content) >= 1:
            ...
            for amenity in content[0].xpath('.//span/del/text()'):
                amenities.append(amenity.strip())
